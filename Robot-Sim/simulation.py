...
def robot_init(dt, body_pos, fixed=False):
    """
    Initialise the robot in the PyBullet simulation environment.
    ...
    joint_ids = [p.getJointInfo(body_id, j)[0] for j in range(p.getNumJoints(body_id))]
...
def robot_stepsim(body_id, body_pos, body_orn, body_to_feet):
    ...
    fr_angles, fl_angles, br_angles, bl_angles, body_to_feet, valid = kinematics.solve(
        body_orn, body_pos, body_to_feet
    )
    if valid:
        # Move movable joints
        print(
            f"FR: {fr_angles * 180/math.pi}, FL: {fl_angles * 180/math.pi}, BR: {br_angles * 180/math.pi}, BL: {bl_angles * 180/math.pi}"
        )
        for i in range(3):
...
if __name__ == "__main__":
    ...
    offset = np.array(
        [0.5, 0.0, 0.0, 0.5]
    )  # Offset between each foot step (FR, FL, BR, BL)
    ...
    for _ in range(num_steps):
        (
            ...
            offset,
        ) = pybullet_debug.update_camera_and_get_robot_states(
            body_id
        )  # User input
        body_to_feet = trot.loop(
            leg_length,
            angle,
            leg_rotation,
            step_period,
            ...
            body_to_feet_init,
            step_duration_asymmetry,
        )  # Calculate feet coordinates for gait

        robot_stepsim(
            body_id, pos, orn, body_to_feet
        )  # Simulate robot to the target position
