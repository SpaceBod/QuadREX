...
class PyBulletDebug:
    def __init__(self):
        ...
        self.params = {
            "pos_x": p.addUserDebugParameter("-Leg-X", -0.02, 0.02, 0.0),
            "pos_y": p.addUserDebugParameter("-Leg-Y", -0.02, 0.02, 0.0),
            "pos_z": p.addUserDebugParameter("-Leg-Z", -0.02, 0.02, 0.0),
            "roll": p.addUserDebugParameter("-Body-Roll", -np.pi / 4, np.pi / 4, 0.0),
            "pitch": p.addUserDebugParameter("-Body-Pitch", -np.pi / 4, np.pi / 4, 0.0),
            "yaw": p.addUserDebugParameter("-Body-Yaw", -np.pi / 4, np.pi / 4, 0.0),
            "leg_length": p.addUserDebugParameter("-Leg-Stride", -0.50, 1, 0.0),
            "leg_rotation": p.addUserDebugParameter("-Leg-Rotate", -1.50, 1.50, 0.0),
            "walk_angle": p.addUserDebugParameter("-Leg-Strafe", -180.0, 180.0, 0.0),
            "step_period": p.addUserDebugParameter("-Leg-Cycle", 0.1, 3.0, 1.0),
            "step_duration_asymmetry": p.addUserDebugParameter(
                "-Step-Dur-Asym", -2, 2.0, 0.0
            ),
            "trot": p.addUserDebugParameter("-Trot", 0, 1, 0),
            "bound": p.addUserDebugParameter("-Step", 0, 1, 0),
        }
    ...
    def update_camera_and_get_robot_states(self, box_id):
        ...
        position = np.array(
            [
                p.readUserDebugParameter(self.params[axis])
        ...
        step_duration_asymmetry = p.readUserDebugParameter(
            self.params["step_duration_asymmetry"]
        )
