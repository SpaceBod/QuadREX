...
def angleMapping(solveLegResult, flip):
    """
    Adjust the angles based on the flip mask, add 90 degrees, and convert to degrees.

    Parameters:
    solveLegResult (tuple): Result from solveLeg, includes angles and valid flag.
    flip (np.array): Boolean mask (length 3), where True means flip the sign of the corresponding angle.

    Returns:
    tuple: Adjusted angles in degrees and validity flag.
    """
    angles, valid = solveLegResult
    if not valid:
        return None, valid
    ...
    return adjusted_angles_degrees, valid
...
def solveFL(coord, coxa, femur, tibia):
    return angleMapping(solveLeg(coord, coxa, femur, tibia, coord1Sign=1, coord2Sign=-1, coxaSign=1, gammaSign=1, alphaAdjustment=np.pi/4.0, gammaAdjustment=-np.pi/2.0), np.array([0, 0, 0]))
