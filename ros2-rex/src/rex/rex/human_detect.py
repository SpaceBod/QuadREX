...
class TrackletsParser(Node):
    ...
    def transform_position(self, x, z):
        try:
            ...
            camera_coords = np.array([z, -x, 0])  # Camera X -> Robot Y, Camera Z -> Robot X
...
def main(args=None):
    ...
    def send_goal_position():
        while not tracklets_parser.detected_goal:
            rclpy.spin_once(tracklets_parser, timeout_sec=1)  # Spin to keep node running and processing detections

        input("Press Enter to send goal position...")
        tracklets_parser.publish_goal()

    rclpy.spin_once(tracklets_parser, timeout_sec=1)  # Spin to initialize node
